// Porté depuis PokedexG.Uwp/Services/VeekunServices/Business/PokemonBusiness.cs.
// RECONSTRUCTION 2026-07-23 — GetWeaknessesAsync devient GetWeaknesses(typeRelations, …) :
// même corps, la lecture Veekun est fournie par l'appelant (la couche données n'est plus
// accessible depuis le navigateur). Les surcharges PokemonUiModel/DamageTypeUiModel de la
// seconde pile (EF Core, non portée) sont omises — code dupliqué à l'identique dans l'original.
using System;
...
namespace PokedexG.Uwp.Services.VeekunServices.Business
{
    public static class PokemonBusiness
    {
        ...
        public static List<DamageType> GetWeaknesses(IEnumerable<TypeRelation> typeRelations,
            int type1Id, int type2Id, bool excludeNeutral = false)
        {
